...
$part_number=$_GET['id'];
$tttt=$_SESSION['tgl'];
$q=mysql_query("Select count(id_keluar) as numrow,sum(jumlah) as jml,jumlah from barang_keluar Where part_number='".$part_number."' and month(tgl)='".($_SESSION['tgl']-1)."'");
...
$sd=array();
$q2=mysql_query("Select * from barang_keluar WHERE part_number='".$part_number."' and month(tgl)='".($_SESSION['tgl']-1)."'");
...
$q1=mysql_query("Select * from data_persediaan where part_number='".$part_number."'");
...
{
	$q=mysql_query("Insert into data_perencanaan (`tgl`, `part_number`, `rata`, `periode`, `lead`, `safety`, `tersedia`, `order`, `eoi`) values ('".date("Y-m-d")."','".$_GET['id']."','".$_POST['a']."','".$_POST['b']."','".$_POST['c']."','".$_POST['d']."','".$_POST['e']."','".$_POST['f']."','".$_POST['g']."')");
	if($q)
	{
		$q2=mysql_query("update data_persediaan SET stok_akhir=stok_tersedia-keluar,rata_keluar='".$_POST['a']."',safety_stok='".$_POST['d']."' Where part_number='".$_GET['id']."'") or die(mysql_error());
		if($q2)
		{
...
if(isset($_POST['button2']))
{
	...
	echo "<script>window.location='?cat=sekretaris&page=quantity&cal=1&id=".$_POST['partnumber']."'</script>";
}
?>
</form>
<?php
